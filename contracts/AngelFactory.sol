...
abstract contract AngelsSupply is Context {
	/*
	 * Public Variables
	 */
...
abstract contract AssignRandomAngel is AngelsSupply {
	...
	/// Initializes the contract
	constructor() AngelsSupply(MAX_ANGELS_SUPPLY) {
		startFrom = 0;
	}
...
contract AngelFactory is IHealGems, MultiAuth, AssignRandomAngel, ReentrancyGuard {
	...
	HealingGems nftContractAddress;
	 ...
	 * Constructor
	 */
	constructor(address operator) MultiAuth(payable(msg.sender)) {
        ...
        nftContractAddress = new HealingGems();
		_tokenBaseURI = baseURI;
        _healAngelsDonationsAddress = payable(address(this));
    ...
    */
    function _baseURI() public view virtual override(HealingGems) returns (string memory) {
        return baseURI;
    }
     ...
     */
    function tokenURI(uint256 tokenId) public view virtual override(HealingGems) returns (string memory) {
